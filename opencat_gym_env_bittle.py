...
class OpenCatGymEnv(gym.Env):
    ...
    def step(self, action):
        ...
        jointAngles[5] = np.clip(jointAngles[5], 0, self.boundAngles  )                  # knee_right
        jointAngles[6] = np.clip(jointAngles[6], -self.boundAngles, self.boundAngles)                      # hip_left
        jointAngles[7] = np.clip(jointAngles[7], 0, self.boundAngles)                     # knee_left
...
#        # Normalize jointAngles
        ...
        jointAngles[1] = (jointAngles[1]-self.boundAngles/2) / self.boundAngles * 2 
        jointAngles[2] /= self.boundAngles
        jointAngles[3] = (jointAngles[3]-self.boundAngles/2) / self.boundAngles * 2                     
        jointAngles[4] /= self.boundAngles
        jointAngles[5] = (jointAngles[5]-self.boundAngles/2) / self.boundAngles * 2                       
        jointAngles[6] /= self.boundAngles          
        jointAngles[7] = (jointAngles[7]-self.boundAngles/2) / self.boundAngles * 2                      
